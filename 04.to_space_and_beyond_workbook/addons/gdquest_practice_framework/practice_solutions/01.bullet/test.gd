...
func is_moving_towards_top_right(frame_previous: FrameData, frame_current: FrameData) -> bool:
	...
	var dot_product: float = position_difference.normalized().dot(DIRECTION_TOP_RIGHT)
	return not position_difference.is_equal_approx(Vector2.ZERO) and abs(dot_product) - 1.0 < 0.1
...
func is_moving_based_on_velocity_and_delta(_frame_previous: FrameData, frame_current: FrameData) -> bool:
	return frame_current.practice_position.distance_squared_to(frame_current.solution_position) < 1.0
...
func is_rotation_incorrect(frame_previous: FrameData, frame_current: FrameData) -> bool:
